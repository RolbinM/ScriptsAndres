...
BEGIN
    ...
    BEGIN TRY
		...
		BEGIN TRANSACTION RepoTarjetaFisica
			-- Insertar datos calculados masivamente en la tabla temporal
			INSERT INTO @MovimientosTemp (idTC, nuevoCodigo, nuevoCCV, Valor, nuevoSaldo, idTipoTCMReposicion, idTCM, idTF, idMov, Descripcion, FechaOperacion, Razon, TF)
			SELECT 
				tf.idTC,
				...
				tf.id,
				tm.id,
				'Recuperacion por ' + inMov.Razon,
				inMov.FechaOperacion,
				inMov.Razon,
				inMov.TF
			FROM @inMovimientos inMov
			INNER JOIN dbo.TF tf ON tf.Codigo = inMov.TF
			...
			INNER JOIN dbo.RN rn ON rn.Nombre = CASE 
												   ...
												   WHEN tcmAux.id IS NOT NULL THEN 'Reposicion de tarjeta de TCA' 
											   END
									AND rn.idTipoTCM = COALESCE(tipoTCM.id, tipoTCMAux.id)
			INNER JOIN dbo.TipoMovimiento tm ON tm.Nombre = 'Recuperacion por ' + inMov.Razon
			WHERE tf.Activa = 'SI';
			...
			SET @outResultCode = 0;
        COMMIT TRANSACTION RepoTarjetaFisica;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
        BEGIN
            ROLLBACK TRANSACTION RepoTarjetaFisica;
        END
